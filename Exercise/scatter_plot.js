function scatter_plot(X, Y, markersize,
    ...
    let colorScale = d3.scaleLinear().domain([0, 1, 2]).range(['steelblue', 'orange', 'brown'])
    let axis = d3.select(`#${axis_key}`);
    //scale for markersizes
    let markerScale = d3.scaleLinear().domain(d3.extent(markersize)).range([2, 10])

    //remove existing before plotting
    ...
    axis.selectAll('.markers')
        ...
        .append('circle')
        // .attr("r", markersize)
        .attr("r", (d, i) => { return markerScale(markersize[i]) })
